...
export default function SelectionSortAnimation({ arr, darkMode, theme }) {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < a.length - 1; i++) {
      ...
      for (let j = i + 1; j < a.length; j++) {
        trace.push({ arr: a.slice(), compare: [minIdx, j], swapped: false, done: false });
        if (a[j] < a[minIdx]) minIdx = j;
      }
      if (minIdx !== i) {
        [a[i], a[minIdx]] = [a[minIdx], a[i]];
        trace.push({ arr: a.slice(), compare: [i, minIdx], swapped: true, done: false });
      }
    }
    trace.push({ arr: a.slice(), compare: [], swapped: false, done: true });
    setActions(trace);
    setStep(0);
    setIsPlaying(true);
  }, [arr]);
  ...
  function getExplanation() {
    ...
    if (curr.compare.length === 2) {
      return `🔎 ${strong(i)}번(${strong(curr.arr[i])})과 ${strong(j)}번(${strong(curr.arr[j])})를 <span style=\"color:#ffd54f;font-weight:700\">비교</span>합니다.`;
    }
    return null;
  }
