...
import ShellSortCode from "../Code/ShellSortCode";
import ShellSortAnimation from "../Animation/ShellSortAnimation";

import useModalBodyScrollLock from '../hooks/useModalBodyScrollLock';

export default function ShellSortModal({ open, onClose, arr, darkMode, theme }) {
    ...
    >
      ...
      >
        ...
        <div style={{
          ...
          셸 정렬 <span style={{ fontSize: 26, color: textColor, fontWeight: 700, letterSpacing: 1 }}>(Shell Sort)</span>
        </div>
        {/* 설명 영역 */}
        <div style={{
          margin: "0 0 20px 0",
          ...
          <span style={{ fontSize: 16.5 }}>
            삽입 정렬의 확장, 간격(gap) 조절<br/>
            <span style={{ color: darkMode ? '#ff8a65' : '#b71c1c', fontWeight: 600 }}>최적의 gap 선택이 중요, 구현이 다소 복잡</span>
          </span>
        </div>
        {/* 초기 배열 영역 */}
        <div style={{
        ...
        <div style={{
          ...
          <ShellSortAnimation arr={arr} darkMode={darkMode} theme={theme} />
        </div>
        {/* 코드 영역 */}
        <div style={{
          width: "100%",
