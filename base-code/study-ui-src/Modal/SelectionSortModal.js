...
import SelectionSortCode from "../Code/SelectionSortCode";
import SelectionSortAnimation from "../Animation/SelectionSortAnimation";
import useModalBodyScrollLock from '../hooks/useModalBodyScrollLock';

export default function SelectionSortModal({ open, onClose, arr, darkMode, theme }) {
  ...
  return (
    <div
      style={{
        position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', background: overlayBg, zIndex: 1200, display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'background 0.3s',
    ...
    >
      ...
      >
        ...
        <div style={{
          ...
          선택 정렬 <span style={{ fontSize: 26, color: textColor, fontWeight: 700, letterSpacing: 1 }}>(Selection Sort)</span>
        </div>
        {/* 설명 영역 */}
        <div style={{
          margin: "0 0 20px 0",
          ...
          <span style={{ fontSize: 16.5 }}>
            작은 배열, 비교 횟수가 적은 데이터<br/>
            <span style={{ color: darkMode ? '#ff8a65' : '#b71c1c', fontWeight: 600 }}>대규모 데이터에는 부적합, 단순 구조가 장점</span>
          </span>
        </div>
        {/* 초기 배열 영역 */}
        <div style={{
        ...
        {/* 그래프 시각화 - SelectionSortAnimation */}
        <div style={{
          width: "100%",
          minHeight: 340,
          ...
          <SelectionSortAnimation arr={arr} darkMode={darkMode} theme={theme} />
        </div>
        {/* 코드 영역 */}
        <div style={{
          width: "100%",
