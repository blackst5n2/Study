...
import MergeSortCode from "../Code/MergeSortCode";
import MergeSortAnimation from "../Animation/MergeSortAnimation";
import useModalBodyScrollLock from '../hooks/useModalBodyScrollLock';

export default function MergeSortModal({ open, onClose, arr, darkMode, theme }) {
    ...
    >
      ...
      >
        ...
        <div style={{
          ...
          병합 정렬 <span style={{ fontSize: 26, color: textColor, fontWeight: 700, letterSpacing: 1 }}>(Merge Sort)</span>
        </div>
        {/* 설명 영역 */}
        <div style={{
          margin: "0 0 20px 0",
          ...
          <span style={{ fontSize: 16.5 }}>
            대규모 데이터, 안정 정렬이 필요한 경우<br/>
            <span style={{ color: darkMode ? '#ff8a65' : '#b71c1c', fontWeight: 600 }}>메모리 사용량 증가, 구현이 비교적 복잡</span>
          </span>
        </div>
        {/* 초기 배열 영역 */}
        <div style={{
        ...
        <div style={{
          ...
          <MergeSortAnimation arr={arr} darkMode={darkMode} theme={theme} />
        </div>
        {/* 코드 영역 */}
        <div style={{
          width: "100%",
