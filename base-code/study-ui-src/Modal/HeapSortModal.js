...
import HeapSortCode from "../Code/HeapSortCode";
import HeapSortAnimation from "../Animation/HeapSortAnimation";
import useModalBodyScrollLock from '../hooks/useModalBodyScrollLock';

export default function HeapSortModal({ open, onClose, arr, darkMode, theme }) {
    ...
    >
      ...
      >
        ...
        <div style={{
          ...
          힙 정렬 <span style={{ fontSize: 26, color: textColor, fontWeight: 700, letterSpacing: 1 }}>(Heap Sort)</span>
        </div>
        {/* 설명 영역 */}
        <div style={{
          margin: "0 0 20px 0",
          ...
          <span style={{ fontSize: 16.5 }}>
            우선순위 큐 구현, 대용량 데이터<br/>
            <span style={{ color: darkMode ? '#ff8a65' : '#b71c1c', fontWeight: 600 }}>안정 정렬이 아님, 구현이 복잡할 수 있음</span>
          </span>
        </div>
        {/* 초기 배열 영역 */}
        <div style={{
        ...
        <div style={{
          ...
          background: darkMode
            ? 'linear-gradient(135deg,#23272f 80%,#181c21 100%)'
            : 'linear-gradient(135deg,#fafdff 80%,#e3f2fd 100%)',
          padding: '20px 0 20px 0',
          position: 'relative',
          maxWidth: 1100,
        }}>
          <HeapSortAnimation arr={arr} darkMode={darkMode} theme={theme} />
        </div>
        {/* 코드 영역 */}
        <div style={{
          width: "100%",
