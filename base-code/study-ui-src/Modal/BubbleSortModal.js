...
import BubbleSortCode from "../Code/BubbleSortCode";

import useModalBodyScrollLock from '../hooks/useModalBodyScrollLock';

export default function BubbleSortModal({ open, onClose, arr, darkMode, theme }) {
    ...
    >
      ...
      >
        ...
        <div style={{
          ...
          버블 정렬 <span style={{ fontSize: 26, color: textColor, fontWeight: 700, letterSpacing: 1 }}>(Bubble Sort)</span>
        </div>
        {/* 설명 영역 */}
        <div style={{
          margin: "0 0 20px 0",
          ...
          <span style={{ fontSize: 16.5 }}>
            작은 배열, 거의 정렬된 데이터<br/>
            <span style={{ color: darkMode ? '#ff8a65' : '#b71c1c', fontWeight: 600 }}>대규모 데이터나 성능이 중요한 경우에는 사용하지 않음</span>
          </span>
        </div>
        {/* 초기 배열 영역 */}
        <div style={{
        ...
        <div style={{
          ...
          <BubbleSortAnimation arr={arr} darkMode={darkMode} theme={theme} />
        </div>
        {/* 코드 영역 */}
        <div style={{
          width: "100%",
